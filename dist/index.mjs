...
var Pair = class {
  ...
  getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    ...
    if (JSBI7.equal(totalSupply.quotient, ZERO)) {
      liquidity = JSBI7.subtract(
        sqrt(JSBI7.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)),
        MINIMUM_LIQUIDITY
      );
    } else {
  ...
  getLiquidityValue(token, totalSupply, liquidity, feeOn = false, kLast) {
    ...
    return CurrencyAmount2.fromRawAmount(
      token,
      JSBI7.divide(JSBI7.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient)
    );
  }
};
